...
function generateOnExitCallback(factoryID, isPythonProcess, factoryUniqueName) {
  return function (error) {
    ...
    debug(
      "process " + _processName + " of factory " + factoryID + " died " + error
    );
...
function generateOnDataCallback(factoryID) {
  return function (data) {
    var _factory = conversionFactory[factoryID];
    data = data.toString();
    debug(`Factory ${factoryID} received data: "${data}"`);
    ...
    if (data === "204") {
      ...
      if (_factory.readyCallback) {
        debug(`Calling readyCallback for factory ${factoryID}`);
    ...
    if (_factory.currentJob) {
      debug(`Processing current job for factory ${factoryID}`);
      if (pythonErrors[data] !== undefined) {
        debug(`Error encountered: ${pythonErrors[data]}`);
        _factory.currentJob.error = new Error(pythonErrors[data]);
      } else {
        debug(`No known error for data: "${data}"`);
        _factory.currentJob.error = null;
      }
    } else {
      debug(`No current job for factory ${factoryID}`);
    }

    debug(`Calling onCurrentJobEnd for factory ${factoryID}`);
...
function detectLibreOffice(additionalPaths) {
  ...
  function _findBinaries(paths, pythonName, sofficeName) {
    ...
    _whichSoffice =
      which.sync(sofficeName, { path: paths.join(":"), nothrow: true }) ||
      which.sync(sofficeName, { nothrow: true }) ||
      null;
