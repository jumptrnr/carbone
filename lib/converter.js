...
var converter = {
  ...
  convertFile: function (
    ...
    console.log("🔄 [CARBONE] convertFile called:", { inputFile, outputType, outputFile });

    if (isLibreOfficeFound === false) {
      console.error("❌ [CARBONE] LibreOffice not found!");
      return callback(
    ...
    console.log("📋 [CARBONE] Job added to queue. Queue length:", jobQueue.length + 1);
    jobQueue.push(_job);
    executeQueue();
  },
...
function addConversionFactory(callback) {
  ...
  console.log("🔧 [CARBONE] Attempting to spawn LibreOffice at:", converterOptions.sofficeExecPath);
  console.log("🔧 [CARBONE] LibreOffice params:", _officeParams);
  ...
  _officeThread.on('error', function(err) {
    console.error("❌ [CARBONE] LibreOffice spawn error:", err.message);
  });

  // Add stdout/stderr logging
  _officeThread.stdout.on('data', function(data) {
    console.log("📄 [CARBONE-SOFFICE] stdout:", data.toString());
  });

  _officeThread.stderr.on('data', function(data) {
    console.log("⚠️ [CARBONE-SOFFICE] stderr:", data.toString());
  });
  ...
  console.log("✅ [CARBONE] LibreOffice thread started with PID " + _officeThread.pid);

  console.log("🔧 [CARBONE] Attempting to spawn Python at:", converterOptions.pythonExecPath);
  console.log("🔧 [CARBONE] Python script:", params.pythonPath);
  ...
  // Add error handler for Python spawn
  _pythonThread.on('error', function(err) {
    console.error("❌ [CARBONE] Python spawn error:", err.message);
  });

  debug("python thread started with PID " + _pythonThread.pid);
  console.log("✅ [CARBONE] Python thread started with PID " + _pythonThread.pid);
  ...
  _pythonThread.stderr.on("data", function (err) {
    ...
    console.log("⚠️ [CARBONE-PYTHON] stderr:", err.toString());
  });
...
function generateOnExitCallback(factoryID, isPythonProcess, factoryUniqueName) {
  return function (error) {
    ...
    console.error("💥 [CARBONE] Process " + _processName + " (PID " + _factory.pid + ") died! Error code:", error);
...
function generateOnDataCallback(factoryID) {
  return function (data) {
    ...
    // Handle JSON response from Python converter (includes extracted images)
    ...
    try {
      ...
      if (responseData.status) {
        ...
        // Store extracted images in the current job for postprocessor access
        if (_factory.currentJob && responseData.images) {
          _factory.currentJob.extractedImages = responseData.images;
        }
      }
    } catch (e) {
      // If JSON parsing fails, treat as plain error code (backward compatibility)
    }

    // Document converted with or without errors
    if (_factory.currentJob) {
      _factory.currentJob.error =
        pythonErrors[errorCode] !== undefined
          ? new Error(pythonErrors[errorCode])
          : null;
    }
...
function onCurrentJobEnd(factory) {
  ...
  if (_job && _job.callback instanceof Function) {
    ...
    // Pass extracted images as a third parameter to the callback
    console.log(
      "DEBUG: onCurrentJobEnd - passing extractedImages:",
      _job.extractedImages ? Object.keys(_job.extractedImages).length : "none"
    );
    _job.callback(_job.error, _job.outputFilePath, _job.extractedImages);
  }
  executeQueue();
}
...
function executeQueue() {
  console.log("⚙️ [CARBONE] executeQueue - Queue length:", jobQueue.length, "Active factories:", activeFactories.length);

  if (jobQueue.length === 0) {
    console.log("✅ [CARBONE] Queue empty, nothing to do");
    return;
  }
  // if there is no active factories, start them
  if (activeFactories.length < params.factories) {
    console.log("🏭 [CARBONE] Starting new factory. Active:", activeFactories.length, "Max:", params.factories);
  ...
  for (var i in conversionFactory) {
    if (jobQueue.length > 0) {
      var _factory = conversionFactory[i];
      console.log("🏭 [CARBONE] Factory", i, "- Ready:", _factory.isReady, "Converting:", _factory.isConverting);
      if (_factory.isReady === true && _factory.isConverting === false) {
        var _job = jobQueue.shift();
        console.log("📤 [CARBONE] Sending job to factory", i);
...
function sendToFactory(factory, job) {
  console.log("📨 [CARBONE] sendToFactory - Starting conversion:", {
    format: job.outputFormat,
    input: job.inputFilePath,
    output: job.outputFilePath,
    attempt: job.nbAttempt + 1
  });
  ...
  console.log("📝 [CARBONE] Python command:", command.trim());

  factory.pythonThread.stdin.write(command);
