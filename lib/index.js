console.log("Starting to load Carbone module...");
...
try {
  ...
  debug("Initializing Carbone module");

  var dayjs = require("dayjs");

  var locales = require("../formatters/_locale");
} catch (error) {
  console.error("Error loading Carbone module dependencies:", error);
  throw error;
}
...
var carbone = {
  ...
  set: function (options) {
    console.log("Setting up Carbone with options:", options);
   ...
   */
  reset: function () {
    console.log("Resetting Carbone parameters...");
      ...
      file.openTemplate(templatePath, function (err, template) {
        ...
        preprocessor.execute(template, options, function (err, template) {
          ...
          walkFiles(template, data, options, 0, function (err, report) {
            //console.log("walkFiles executed");
            if (err) {
              return callback(err, null);
            }

            postprocessor.process(template, data, options, function (err) {
              //console.log("postprocessor executed");
              if (err) {
                return callback(err, null);
              }
              ...
              file.buildFile(report, function (err, result) {
                //console.log("file.buildFile executed");
                if (err) {
                  return callback(err, null);
                }
                convert(
                  ...
                  report.reportName,
                  options,
                  function (err, bufferOrFile) {
                    if (
                      report.reportName === undefined &&
                      typeof bufferOrFile === "string"
                    ) {
                    ...
                    callback(
                      ...
                      bufferOrFile,
                      report.reportName,
                      options.isDebugActive === true ? options.debugInfo : null
                    );
                  }
                );
              });
  ...
  decodeRenderedFilename: function (pathOrFilename, prefixLength = 0) {
    ...
    var _onlyReportName = _filename.slice(
      prefixLength + helper.RANDOM_STRING_LENGTH,
      -_extension.length
    );
  ...
  convert: function (fileBuffer, optionsRaw, callbackRaw) {
    input.parseOptions(optionsRaw, callbackRaw, function (options, callback) {
      ...
      if (options.extension === null) {
        return callback(
          "Unknown input file type. options.extension should be equals to docx, xlsx, pptx, odt, ods, odp, xhtml, html or an xml"
        );
      }
      var _error = input.parseConvertTo(options, optionsRaw.convertTo);
      if (_error) {
...
function convert(inputFileBuffer, customReportName, options, callback) {
  ...
  const _hasConversion =
    options.extension !== _convertTo.extension || options.hardRefresh === true;
  const _isReturningBuffer =
    options.renderPrefix === undefined || options.renderPrefix === null;
  ...
  const _renderPrefix = (options.renderPrefix || "").replace(
    /[^0-9a-z-]/gi,
    ""
  );
  const _randomNamePart = helper.getRandomString();
  const _customReportName =
    customReportName !== undefined ? customReportName : "report";
  const _renderFilename =
    _renderPrefix +
    _randomNamePart +
    helper.encodeSafeFilename(_customReportName) +
    "." +
    _convertTo.extension;
  ...
  const _intermediateFilename =
    _renderPrefix + _randomNamePart + "_tmp." + options.extension;
  ...
  fs.writeFile(_intermediateFile, inputFileBuffer, function (err) {
    ...
    converter.convertFile(
      _intermediateFile,
      _convertTo.format,
      _convertTo.optionsStr,
      _renderFile,
      function (errConvert, outputFile, extractedImages) {
        ...
        fs.readFile(outputFile, function (err, outputBuffer) {
          ...
          // If we have extracted images and this is an HTML conversion,
          // we need to process them through the postprocessor
          if (extractedImages && _convertTo.format === "html") {
            console.log(
              "DEBUG: Found extracted images for HTML processing:",
              Object.keys(extractedImages).length
            );
            var postprocessor = require("./postprocessor");

            // Create a template object structure that the postprocessor expects
            var template = {
              extension: "html", // Set extension to html so HtmlPostProcessor is used
              files: [
                {
                  name: outputFile,
                  data: outputBuffer.toString("utf8"),
                },
              ],
            ...
            debug(
              "Processing HTML with extracted images: " +
                Object.keys(extractedImages).length +
                " images"
            );

            // Process the HTML with extracted images
            postprocessor.process(template, {}, options, function (err) {
              if (err) {
                debug("Postprocessor error: " + err);
                return callback(null, outputBuffer); // Return original on error
              }

              // Return the processed HTML buffer
              var processedBuffer = Buffer.from(template.files[0].data, "utf8");
...
carbone.addFormatters({
  drop: function (data, args) {
    ...
    return data; // Return the data unchanged if the condition is not met
  },
});
translator.loadTranslations(params.templatePath);
