...
class html2xml {
  ...
  getXML() {
    ...
    if (!this.hasError && this.json) {
      ...
      if (lastResult.endsWith(NEW_PARAGRAPH))
        this.result.push(
          lastResult.substring(0, lastResult.length - NEW_PARAGRAPH.length)
        );
      else {
      ...
      if (this.hasTable)
        ...
        temp.push(
          this.result.join("").replaceAll("<w:p><!--table--></w:p>", "")
        );
      else temp.push(this.result.join(""));
    } else {
  ...
  buildStyles(styles) {
    const temp = Array.isArray(styles)
      ? styles
  ...
  isEmptyParagraph(element) {
    if (element.node === "text" && typeof element.text === "string")
      return false;
  ...
  setProps(child, deep = 0) {
    if (child.tag === "p") {
      if (child.attr?.class) {
        const [first] = Array.isArray(child.attr.class)
          ? child.attr.class
          : [child.attr.class];
      ...
      child.properties.hasBorder =
        child.attr?.border && child.attr.border !== "0";
  ...
  buildText(child) {
    ...
    if (props) {
        ...
        for (var arg in styles) {
          if (styles.hasOwnProperty(arg)) {
            ...
            if (arg === "color") {
              ...
              if (rgb !== null) {
                text.push(
                  `<w:shd w:val="clear" w:color="33FF49" w:fill="${rgb.toUpperCase()}"/>`
                );
              }
            }
            if (arg === "font-size") {
            ...
            if (arg === "text-indent") {
              ...
              if (pc)
                props.formatting_options = {
                  ...props.formatting_options,
                  indent: Math.round(72 * parseFloat(pc)),
                };
            }
            // 12pt = 240 unit
            if (arg === "margin-top") {
              ...
              if (pt)
                props.formatting_options = {
                  ...props.formatting_options,
                  spacingBefore: Math.round((240 * parseFloat(pt)) / 12),
                };
            }
            if (arg === "margin-bottom") {
              ...
              if (pt)
                props.formatting_options = {
                  ...props.formatting_options,
                  spacingAfter: Math.round((240 * parseFloat(pt)) / 12),
                };
            }
            if (arg === "line-height") {
              const [, pt] = val.match(/^(\d+(.\d+)?)pt$/) || [];
              if (pt)
                props.formatting_options = {
                  ...props.formatting_options,
                  spacingLine: Math.round((240 * parseFloat(pt)) / 12),
                };
            }
          }
        }
      ...
      if (props.formatting_options) {
        ...
        fopts.push(
          `<w:pStyle w:val="${
            props.formatting_options.style || this.pstyle || "Normal"
          }"/>`
        );
        if (
          "spacingBefore" in props.formatting_options ||
          "spacingAfter" in props.formatting_options ||
          "spacingLine" in props.formatting_options
        ) {
          let spacing = "";
          if ("spacingLine" in props.formatting_options)
            spacing += ` w:line="${props.formatting_options.spacingLine}"`;
          if ("spacingBefore" in props.formatting_options)
            spacing += ` w:before="${props.formatting_options.spacingBefore}"`;
          if ("spacingAfter" in props.formatting_options)
            spacing += ` w:after="${props.formatting_options.spacingBefore}"`;
          fopts.push(`<w:spacing ${spacing}/>`);
        }
        if (props.formatting_options.indent)
          fopts.push(
            `<w:ind w:left="${props.formatting_options.indent}" w:hanging="0"/>`
          );
      ...
      if (props.table) {
        ...
        if (props.col === 0 && props.row === 0) {
          ...
          // Reset table border settings for each new table
          if (props.hasBorder) {
            this.result.push(`
                <w:tblBorders>
                  ...
                  <w:insideH w:val="single" w:sz="${
                    wsz - 2
                  }" w:space="0" w:color="000000" />
                  <w:insideV w:val="single" w:sz="${
                    wsz - 2
                  }" w:space="0" w:color="000000" />
                </w:tblBorders>
              `);
          } else {
            // Explicitly set no borders if hasBorder is false
            this.result.push(`
                <w:tblBorders>
