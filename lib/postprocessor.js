...
class HtmlPostProcessor extends Processor {
  ...
  processImages(callback) {
    debug("Starting HTML image processing");
    const htmlFile = this.template.files[0];
    if (!htmlFile) {
      //debug("No HTML file found, skipping image processing");
      return callback();
    }

    // Find all img tags with HTTP/HTTPS URLs AND local file references
    const httpImgTagRegex =
      /<img\s+[^>]*src=["'](https?:\/\/[^"']+)["'][^>]*>/gi;
    const localImgTagRegex =
    ...
    while ((match = httpImgTagRegex.exec(htmlFile.data)) !== null) {
      const url = match[1];
      if (url && !httpImageUrls.includes(url)) {
        httpImageUrls.push(url);
      }
    }

    // Find local image file references (reset regex)
    const localRegex =
      /<img\s+([^>]*?)src=["']([^"']+\.(png|jpg|jpeg|gif|svg))["']([^>]*?)>/gi;
    while ((match = localRegex.exec(htmlFile.data)) !== null) {
      const filePath = match[2];
      if (filePath && !localImageFiles.includes(filePath)) {
        localImageFiles.push(filePath);
      }
    }

    debug(
      `Found ${httpImageUrls.length} HTTP image URLs and ${localImageFiles.length} local image files to process in HTML`
    );

    // Process HTTP URLs if any exist
    const processHttpImages = (httpUrls) => {
      if (httpUrls.length === 0) {
        return Promise.resolve();
      }

      return fetchImages(httpUrls).then((resolved) => {
        this.filestore.imagesFetched(resolved.map((r) => r.value));

        let processedImages = 0;
        htmlFile.data = htmlFile.data.replace(
          httpImgTagRegex,
          (imgTag, url) => {
            ...
            processedImages++;
            return imgTag.replace(/src=["'][^"']+["']/, `src="${dataUri}"`);
          }
        );

        debug(`Processed ${processedImages} HTTP images in HTML file`);
        return processedImages;
      });
    };

    // Process local images using extracted images from LibreOffice converter
    const processLocalImages = (localFiles) => {
      ...
      // Check if we have extracted images from the converter
      const extractedImages = this.template.extractedImages || {};

      localFiles.forEach((filePath) => {
        // Check if this file was extracted by LibreOffice
        if (extractedImages[filePath]) {
          const imageData = extractedImages[filePath];
          const dataUri = `data:${imageData.mime};base64,${imageData.data}`;

          // Replace this specific image reference
          const escapedFilePath = filePath.replace(
            /[.*+?^${}()|[\]\\]/g,
            "\\$&"
          );
          const regex = new RegExp(
            `(<img\\s+[^>]*?)src=["']${escapedFilePath}["']([^>]*?>)`,
            "gi"
          );

          htmlFile.data = htmlFile.data.replace(regex, `$1src="${dataUri}"$2`);
          processedLocalImages++;
          debug(`Processed extracted image: ${filePath}`);
        } else {
          // Try to find a corresponding file in the template files (fallback for embedded images)
          const imageFile = this.template.files.find(
            (file) =>
              file.name.includes(filePath.split("_html_")[0]) ||
              file.name.endsWith(filePath) ||
              (filePath.includes("_html_") &&
                file.name.includes(filePath.split("_html_")[1]))
          );
          ...
          if (imageFile && imageFile.data) {
            ...
            const content = Buffer.isBuffer(imageFile.data)
              ? imageFile.data.toString("base64")
            ...
            processedLocalImages++;
            debug(`Processed embedded image: ${filePath}`);
          }
        }
      });

      if (processedLocalImages === 0 && localFiles.length > 0) {
        debug(
          `Warning: ${localFiles.length} local image files could not be processed`
        );
        debug(`Local image files: ${localFiles.join(", ")}`);
      }

      return Promise.resolve(processedLocalImages);
    };

    // Process both HTTP and local images
    return processHttpImages(httpImageUrls)
      .then(() => processLocalImages(localImageFiles))
      .then((processedLocalImages) => {
        debug(`Total local images processed: ${processedLocalImages}`);

        if (localImageFiles.length > processedLocalImages) {
          debug(
            `Note: ${
              localImageFiles.length - processedLocalImages
            } local images could not be processed`
          );
        }
      ...
      .catch((e) => {
        console.error("Error during HTML image processing:", e);
        callback(new Error("HTML image processing error"));
      });
  }
  processHTML(callback) {
    try {
