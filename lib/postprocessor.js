...
class OdpPostProcessor {
  ...
  processContent() {
    ...
    contentXml.data = contentXml.data.replace(drawFrameRegex, (drawFrame) => {
      ...
      // Look for base64 image data in <svg:title> or <svg:desc>
      const base64Match = drawFrame.match(
        /<svg:(title|desc)>data:([^;]+);base64,([^<]+)<\/svg:\1>/
      );

      if (!base64Match) {
        console.log("No base64 image data found in drawFrame");
        return drawFrame;
      }

      const [, , mime, content] = base64Match;
      console.log(`Base64 image found with MIME type: ${mime}`);
      ...
      drawFrame = drawFrame
        ...
        .replace(/<svg:(title|desc)>data:[^<]+<\/svg:\1>/, `<svg:$1></$1>`)
        ...
        // If there's no draw:image element, add it
        .replace(
          /<\/draw:frame>/,
