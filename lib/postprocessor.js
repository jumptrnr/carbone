...
class FileStore {
  ...
  fileBaseName(base64) {
    if (!base64 || !Buffer.isBuffer(base64)) {
      console.log("Invalid image data provided to fileBaseName");
      return [null, false];
    }
...
function fetchImages(urls) {
  ...
  const fetches = [...new Set(urls || [])]
    .map((url) => {
      const fixed_url = url
        .replace(/&amp;/g, "&")
        .replace(/&#x[A-F0-9]+;/gi, "") // Remove ALL hex HTML entities, not just specific ones
        .replace(/&#\d+;/g, "") // Remove ALL decimal HTML entities
        .replace(/[\n\r\t]/g, "") // Remove newlines, carriage returns, and tabs
        .replace(/\s+/g, "%20") // Replace spaces with %20
        .trim();

      // Log any significant URL changes
      if (url !== fixed_url) {
        console.log(`URL cleaned from "${url}" to "${fixed_url}"`);
      }
    ...
    .map(({ url, fixed_url }) =>
      axios({
        ...
        .then(({ data, headers }) => {
          ...
          console.log(`Successfully fetched image from ${fixed_url}:`, {
            contentType,
...
class DocxPostProcessor extends Processor {
  ...
  processImages(callback) {
    ...
    console.log("Searching for drawings in document...");
    ...
    const drawingUrls = allDrawings
      .map((drawing) => {
        ...
        if (!urlMatch) {
          console.log("No URL found in drawing");
          return null;
        }
        ...
        console.log("Found image reference:", {
          url,
          relationshipId,
        });
    ...
    console.log(`Found ${drawingUrls.length} image URLs in drawings`);

    const uniqueUrls = [...new Set(drawingUrls.map((item) => item.url))];
    console.log("Unique URLs to fetch:", uniqueUrls);
      ...
      .then((resolved) => {
        ...
        this.documentXmlFile.data = this.documentXmlFile.data.replaceAll(
          ...
          (drawing) => {
            ...
            if (!urlMatch) {
              console.log("Keeping original drawing (no URL found)");
              return drawing;
            }
            ...
            if (!content || !mime || !oldRelationshipId) {
              console.log(`Skipping drawing due to missing data:`, {
                hasContent: !!content,
                mime,
                oldRelationshipId,
                imageUrl,
        ...
        console.log(
          `Processed ${processedRelationships.size} unique relationships`
        );
        console.log(
